...
import com.gorga.frontend.commands.RestartCommand;
...
public class Main extends Game {
    ...
    private void update(float delta) {
        if (Gdx.input.isKeyPressed(Input.Keys.SPACE)) jetpackCommand.execute();

        if (player.isDead()) {
            if (Gdx.input.isKeyJustPressed(Input.Keys.SPACE)) restartCommand.execute();
            return;
        }
    ...
    private void renderGame(ShapeRenderer shapeRenderer) {
        ...
        scoreUIObserver.render(spriteBatch, gameManager.getScore());
    }
